...
BOOL SettingsDlg::OnInitDialog()
{
	...
	GetDlgItem(IDC_CUSTOM_INI)->SetWindowText(str);
#endif
...
#endif
#ifdef _GLOBAL_CUSTOM_INI
	ON_BN_CLICKED(IDC_BROWSE_INI, &SettingsDlg::OnBnClickedBrowseIni)
	ON_EN_CHANGE(IDC_CUSTOM_INI, &SettingsDlg::OnEnChangeCustomIni)
	ON_BN_CLICKED(IDC_DEFAULT_INI, &SettingsDlg::OnBnClickedDefaultIni)
#endif
#ifndef _GLOBAL_NO_RINGING_SOUND
...
#ifdef _GLOBAL_CUSTOM_INI
	CString str;
	GetDlgItem(IDC_CUSTOM_INI)->GetWindowText(str);
	str.Trim();
	if (str!=accountSettings.pathRoaming) {
		CRegKey regKey;
		CString rab;
		rab.Format(_T("Software\\%s"), _T(_GLOBAL_NAME));
		if (regKey.Create(HKEY_CURRENT_USER,rab)==ERROR_SUCCESS) {
			if (!str.IsEmpty()) {
				regKey.SetStringValue(_T("iniPath"),str);
			} else {
				regKey.DeleteValue(_T("iniPath"));
			}
			regKey.Close();
		}
		accountSettings.Init();

		microsipDlg->pageDialer->DialedClear();
		microsipDlg->pageDialer->DialedLoad();
#ifndef _GLOBAL_NO_CONTACTS
		microsipDlg->pageContacts->ContactClear();
		microsipDlg->pageContacts->ContactsLoad();
#endif
#ifndef _GLOBAL_NO_CALLS
		microsipDlg->pageCalls->CallsClear();
		microsipDlg->pageCalls->CallsLoad();
#endif
	}
#endif
...
#ifdef _GLOBAL_CUSTOM_INI
void SettingsDlg::OnBnClickedBrowseIni()
{
	BROWSEINFO bi = {0};
    ...
    if (pidl != 0) {
		...
		GetDlgItem(IDC_CUSTOM_INI)->SetWindowText(path);
        // free memory used
        IMalloc * imalloc = 0;
...
void SettingsDlg::OnEnChangeCustomIni()
{
	CString str;
	GetDlgItem(IDC_CUSTOM_INI)->GetWindowText(str);
	GetDlgItem(IDC_DEFAULT_INI)->EnableWindow(str.GetLength()>0);
}

void SettingsDlg::OnBnClickedDefaultIni()
{
	GetDlgItem(IDC_CUSTOM_INI)->SetWindowText(_T(""));
}
#endif
